...
本次教程，我们实现了长江流域近十年降雨量的地图可视化与图表可视化，实现了 GEE 平台与本地 Python 的数据分析工具充分结合，实现了更高自由化的遥感分析操作，同时也通过调用 API 的方式缓解了 GEE 对于用户的部分算力限制。在后续的几期教程中，我们将继续以 ==***长江流域***== 为研究区域，计算水量平衡中的各个主要组成部分，并将其与长江流域的植被生长、干旱信息和土地覆盖信息联系起来。
